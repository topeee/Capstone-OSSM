...
if (isset($_POST['export_excel'])) {
    ...
    $header = ['ID', 'First Name', 'Last Name', 'Middle Name', 'DOB', 'Gender', 'Mobile Number', 'Email', 'House Number', 'Street', 'Barangay', 'Role'];
    $sheet->fromArray($header, NULL, 'A1');

    $sql = "SELECT id, first_name, last_name, dob, gender, mobile_number, email, house_number, street, barangay, is_admin FROM users";
...
if (isset($_POST['export_pdf'])) {
    ...
    $html .= '</tr></thead><tbody>';
    $sql = "SELECT id, first_name, last_name, middle_name, dob, gender, mobile_number, email, house_number, street, barangay, is_admin FROM users";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $html .= '<tr>';
...
$sql = "SELECT id, first_name, middle_name, last_name, dob, gender, mobile_number, email, house_number, street, barangay, is_admin FROM users"; // Adjust the table and column names as needed
$result = $conn->query($sql);
