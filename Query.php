<?php

session_start(); // Start the session
include 'header.php';

// Assuming $conn is your active database connection

$sql = "SELECT RefNumber FROM ReferenceNumbers WHERE RefNumber = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $refNumber);
$stmt->execute();
$stmt->bind_result($retrievedRefNumber);


if ($stmt->fetch()) {
    echo "Reference Number: " . $retrievedRefNumber;
}

?>

<html>
    <head>
      <meta charset="UTF-8" />
      <meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no">
      <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/css/bootstrap.min.css" rel="stylesheet">
      <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
      <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha3/dist/js/bootstrap.bundle.min.js"></script>
      <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons/font/bootstrap-icons.css" rel="stylesheet">
      <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.js"></script>
      <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.10.377/pdf.min.js"></script>
      <link rel="stylesheet" href="bootstrap.min.css">
      <link rel="stylesheet" href="bootstrap.min.js">
      <link rel="stylesheet" href="Footer.Clean.icons.css">
      <link rel="stylesheet" href="footer.css">
      <link rel="stylesheet" href="solo parent app.css">
      <link rel="icon" type="img/png" href="logo.png">
      <title>OSSM Query</title>

      <style>
        .custom-input {
          max-width: 500px;
          height: 50px;
          border-radius: 25px;
          text-align: center;
        }
      
        .custom-search-btn {
          background-color: #6c8496;
          color: white;
          border-radius: 25px;
          padding: 10px 40px;
          font-weight: bold;
        }
      
        .custom-search-btn:hover {
          background-color: #5a7183;
        }
      
        .custom-width {
            max-width: 400px; /* Adjust this value as needed */
            margin: 0 auto; /* Center the card */
        }

        .option-card {
          width: 250px;
          border-radius: 15px;
        }
      
        .option-card .btn {
          width: 100%;
          border-radius: 15px;
        }
      </style>

<body>
    <main>
      <div class="container my-4 text-center">
        <h3 class="mb-4">Enter Reference Number</h3>
        <div class="input-group justify-content-center mb-3">
          <input type="text" class="form-control custom-input" placeholder="[ A-XXX-XXXXX ]" aria-label="Reference Number">
        </div>
        <button class="btn custom-search-btn" type="button" data-bs-toggle="modal" data-bs-target="#verifyModal">SEARCH</button>
      </div>
    </main>

     <!-- Verification Modal -->
     <div class="modal fade" id="verifyModal" tabindex="-1" aria-labelledby="verifyModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-body text-center">
              <p>Please verify that you are requesting for the status of</p>
              <p style="font-size: 20px; font-weight: bold;">Username</p>
              <p>Application</p>
              <div class="d-flex justify-content-center mt-3">
                <button type="button" class="btn btn-primary me-3" data-bs-dismiss="modal" onclick="showApplicationStatus()">YES</button>
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">NO</button>
              </div>
            </div>
          </div>
        </div>
      </div>

      <?php
      if ($stmt->fetch()) {
        echo "<script>document.getElementById('ApplicationStatus').style.display = 'block';</script>";
        echo "Reference Number: " . $retrievedRefNumber;
      } else {
        echo "<script>document.getElementById('ApplicationStatus').style.display = 'block';</script>";
        echo "<script>document.getElementById('NameOfOwner').innerText = 'Reference Number Not Found!';</script>";
        echo "<script>document.getElementById('TypeOfApplication').style.display = 'none';</script>";
        echo "<script>document.getElementById('Status').style.display = 'none';</script>";
      }
      ?>
      <!-- Application Status (Initially Hidden) -->
      <div class="container my-4" id="ApplicationStatus" style="display: none;">
        <div class="card mb-4 custom-width">
          <div class="card-body">
        <h5 class="card-title" style="text-align: center;">Application Status</h5>
        <p id="NameOfOwner"><strong>Name of Owner:</strong> USERNAME</p>
        <p id="TypeOfApplication"><strong>Type of Application:</strong> Scholarhip</p>
        <p id="Status"><strong>Status: </strong>On Going</p>
        <p><small><i>Status may vary depending on Head Office</i></small></p>
          </div>
        </div>
      </div>

      <footer>
            <div class="container">
                <div class="row row-cols-1 row-cols-lg-3">
                    <ul class="list-inline my-2">
                        <li class="list-inline-item"><a class="link-secondary" href="#">About us</a></li>
                    </ul>
                    <div class="col">
                        <ul class="list-inline my-2">
                            <li class="list-inline-item me-4">
                                <div class="bs-icon-circle bs-icon-primary bs-icon">
                                    <a href="https://www.facebook.com/sanmateorizalPIO" target="_blank">
                                        <svg class="bi bi-facebook" xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" fill="currentColor" viewBox="0 0 16 16">
                                            <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z"></path>
                                        </svg>
                                    </a>
                                </div>
                            </li>
                            <!-- Increased size for YouTube icon -->
                            <li class="list-inline-item me-4">
                                <div class="bs-icon-circle bs-icon-primary bs-icon" >
                                    <a href="https://www.youtube.com/@SanMateoRizal" target="_blank">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" fill="currentColor" class="bi bi-youtube" viewBox="0 0 16 16" style="vertical-align: middle;">
                                            <path d="M8.051 1.999h.089c.822.003 4.987.033 6.11.335a2.01 2.01 0 0 1 1.415 1.42c.101.38.172.883.22 1.402l.01.104.022.26.008.104c.065.914.073 1.77.074 1.957v.075c-.001.194-.01 1.108-.082 2.06l-.008.105-.009.104c-.05.572-.124 1.14-.235 1.558a2.01 2.01 0 0 1-1.415 1.42c-1.16.312-5.569.334-6.18.335h-.142c-.309 0-1.587-.006-2.927-.052l-.17-.006-.087-.004-.171-.007-.171-.007c-1.11-.049-2.167-.128-2.654-.26a2.01 2.01 0 0 1-1.415-1.419c-.111-.417-.185-.986-.235-1.558L.09 9.82l-.008-.104A31 31 0 0 1 0 7.68v-.123c.002-.215.01-.958.064-1.778l.007-.103.003-.052.008-.104.022-.26.01-.104c.048-.519.119-1.023.22-1.402a2.01 2.01 0 0 1 1.415-1.42c.487-.13 1.544-.21 2.654-.26l.17-.007.172-.006.086-.003.171-.007A100 100 0 0 1 7.858 2zM6.4 5.209v4.818l4.157-2.408z"/>
                                        </svg>
                                    </a>
                                </div>
                            </li>
                            <li class="list-inline-item">
                                <div class="bs-icon-circle bs-icon-primary bs-icon">
                                    <a href="https://www.sanmateo.gov.ph/?fbclid=IwY2xjawGa8FxleHRuA2FlbQIxMAABHV7oEXf9A30xAMK0rZkUq2u79JjY-rg8Nx1htqvExUJJk_Bm0eYPp6P6RA_aem_EAarsNJrWise3DgKGtmDTg" target="_blank">
                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" fill="currentColor" viewBox="0 0 16 16">
                                            <path d="M0 8a8 8 0 1 1 16 0A8 8 0 0 1 0 8m7.5-6.923c-.67.204-1.335.82-1.887 1.855A8 8 0 0 0 5.145 4H7.5zM4.09 4a9.3 9.3 0 0 1 .64-1.539 7 7 0 0 1 .597-.933A7.03 7.03 0 0 0 2.255 4zm-.582 3.5c.03-.877.138-1.718.312-2.5H1.674a7 7 0 0 0-.656 2.5zM4.847 5a12.5 12.5 0 0 0-.338 2.5H7.5V5zM8.5 5v2.5h2.99a12.5 12.5 0 0 0-.337-2.5zM4.51 8.5a12.5 12.5 0 0 0 .337 2.5H7.5V8.5zm3.99 0V11h2.653c.187-.765.306-1.608.338-2.5zM5.145 12q.208.58.468 1.068c.552 1.035 1.218 1.65 1.887 1.855V12zm.182 2.472a7 7 0 0 1-.597-.933A9.3 9.3 0 0 1 4.09 12H2.255a7 7 0 0 0 3.072 2.472M3.82 11a13.7 13.7 0 0 1-.312-2.5h-2.49c.062.89.291 1.733.656 2.5zm6.853 3.472A7 7 0 0 0 13.745 12H11.91a9.3 9.3 0 0 1-.64 1.539 7 7 0 0 1-.597.933M8.5 12v2.923c.67-.204 1.335-.82 1.887-1.855q.26-.487.468-1.068zm3.68-1h2.146c.365-.767.594-1.61.656-2.5h-2.49a13.7 13.7 0 0 1-.312 2.5m2.802-3.5a7 7 0 0 0-.656-2.5H12.18c.174.782.282 1.623.312 2.5zM11.27 2.461c.247.464.462.98.64 1.539h1.835a7 7 0 0 0-3.072-2.472c.218.284.418.598.597.933M10.855 4a8 8 0 0 0-.468-1.068C9.835 1.897 9.17 1.282 8.5 1.077V4z"/>
                                          </svg>
                                    </a>
                                </div>
                            </li>
                        </ul>
                    </div>
                    <div class="col">
                        <ul class="list-inline my-2">
                            <li class="list-inline-item"><a class="link-secondary" href="#">Contact Us</a></li>
                        </ul>
                    </div>
                </div>
            </div>            
        </footer>

    <script>
      function showApplicationStatus() {
        document.getElementById("ApplicationStatus").style.display = "block";
      }
    </script>
</body>
</html>